...
solveOLG <- function(starttime = 1, maxiter = 200, tol = 1e-4, damping_budget = 1.0, damping_assets = 1.0, damping_ab = 1.0, damping_r = 0.5, damping_new_assets = 0.7) {
  ...
  for (iter in 1:maxiter) {
    ...
    cat(paste0("Iteration:  ", formatC(iter,width=3) ,"   scaleA: ", format_dec(scaleA,6), "   scaleab: ", format_dec(mean(scaleab),6), "   non-conv.HH: ", format_dec(sum(HH_nonconvz),0), "   Time: ",  format_dec(tociter$toc-tociter$tic,3), " sec   log of err/tol: ", format_dec(err2,8),"\n"));
